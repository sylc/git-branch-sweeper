...
export function excludeBlacklist(branchName: string, blackList: string[]): boolean {
...
async function gitListBranches(opts: string, remote = false) {
  ...
  try {
    if (remote) {
      console.log(chalk.green(`Retrieving branches with pattern: ${myBranchSelection}`));
    }
    ...
    const myBranches = branchList.filter((branchName) => {
      ...
      if (excludeBlacklist(branchName, blackList)) return false;
  ...
  try {
    ...
    switch (repoType.repo){
      case 'local':
        ...
        const localNoMergedBranches = await gitListBranches('-v --no-merged', false);
      ...
      case 'remote':
        ...
        const allMyRemoteBranches: string[] = await gitListBranches('-v -a --merged', true);
